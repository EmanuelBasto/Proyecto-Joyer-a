...
export default function Sidebar({ className }: { className?: string }) {
  ...
  const navigationItems = [
    { name: "Página principal", path: "/", icon: <Database className="w-6 h-6" /> },
    { name: "Vehículos", path: "/vehicles", icon: <Car className="w-6 h-6" /> },
    { name: "Registros", path: "/records", icon: <FileText className="w-6 h-6" /> },
    { name: "Gráficos", path: "/calendar", icon: <Calendar className="w-6 h-6" /> },
  ];

  return (
    <div className={cn("h-screen bg-[#C78F3D] text-white transition-all duration-300 relative", 
      className, 
      collapsed ? "w-20" : "w-64")}>
      <div className="p-4 flex justify-between items-center border-b border-flotatrack-900">
        <div className={cn("flex items-center", collapsed ? "justify-center" : "")}>
          {!collapsed ? (
            <h1 className="text-xl font-bold">Ejad</h1>
          ) : (
